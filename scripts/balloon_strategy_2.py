...
class BalloonStrategy(object):
    def __init__(self):
        ...
        self.search_yaw_speed = balloon_config.config.get_float('general','SEARCH_YAW_SPEED',3.0) 
        ...
        # use the OpenMV to get tracking
        self.use_OpenMV = balloon_config.config.get_boolean('general','OpenMV',False)     
    ...
    def check_video_out(self):
        ...
        # start video once vehicle is armed
            ...
            #print "Vehicle Armed - Starting Video"
    ...
    def check_status(self):
        ...
        if self.vehicle.mode.name == "GUIDED":
            if not self.controlling_vehicle:
                ...
                self.mission_alt_max = 60
                self.mission_distance_max = 70
    ...
    def analyze_image(self):
        ...
        # look for balloon in image using blob detector === OR USEN OPENMV and divide Radius by factor
        if self.use_OpenMV:
            self.balloon_found, xpos, ypos, size = open_mv.balloon_xysize()
        else:
            self.balloon_found, xpos, ypos, size = balloon_finder.analyse_frame(f)       
        #print self.balloon_found, xpos, ypos, size
        ...
        if self.balloon_found:
            ...
            #PAPOU_MOD == Remove pixel to angle for simulation tests
            self.balloon_pitch_top = self.balloon_pitch + balloon_video.pixels_to_angle_y(size/20)  # add balloon radius so we aim for top of balloon
            self.balloon_heading = math.radians(self.balloon_heading)

            # get distance
            self.balloon_distance = get_distance_from_pixels((size/10), balloon_finder.balloon_radius_expected)
